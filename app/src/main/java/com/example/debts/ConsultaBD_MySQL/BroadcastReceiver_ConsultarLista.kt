...
class BroadcastReceiver_ConsultarLista : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        //CustomToast().showCustomToast(context, "Consultas realizadas!")
        ...
        if (intent != null){
            if (nomeAlarme == "listaMetas") {
                ...
                val ultimaConsultaListaMetas: LocalDateTime =
                    DadosUsuario_BD_Debts(context).getLastUpdateTimestamp_ListaMySQL("Metas")
                ...
                executorService.execute {
                    try {
                        ...
                        if (novaConsultaListaMetas > ultimaConsultaListaMetas) {
                            ...
                            DadosUsuario_BD_Debts.listas_MySQL.metasUsuario =
                                Flask_Consultar_MySQL(context).listarMetas(IDusuario)

                            // Atualiza o timestamp da última consulta
                            DadosUsuario_BD_Debts(context).setLastUpdateTimestamp_ListaMySQL(
                                novaConsultaListaMetas,
                                "Metas"
                            )
                            ...
                            Log.d(
                                "BroadcastReceiver_Metas",
                                "Lista de metas atualizada com sucesso."
                            )
                        } else {
                            metasNovas = false
                        ...
                        handler.post {
                            ...
                            if (metasNovas) {
                                CompararListas_MySQL_SQLite(context).adicionarNovasMetas(
                                    DadosUsuario_BD_Debts.listas_MySQL.metasUsuario,
                                    BancoDados(context).listarMetas(IDusuario)
                                )
                ...
                // Pegando o timestamp da última consulta
                val ultimaConsultaListaGasto: LocalDateTime =
                    DadosUsuario_BD_Debts(context).getLastUpdateTimestamp_ListaMySQL("Gastos")
                ...
                executorService.execute {
                    try {
                        // Fazendo uma nova consulta ao banco de dados
                        val novaConsultaListaGastos: LocalDateTime =
                            Flask_Consultar_MySQL(context).getUltimaAtualizacaoListas_MySQL(
                                IDusuario,
                                "Gastos"
                            )
                        ...
                        if (novaConsultaListaGastos > ultimaConsultaListaGasto) {
                            // Atualiza a lista de metas
                            DadosUsuario_BD_Debts.listas_MySQL.gastosUsuario =
                                Flask_Consultar_MySQL(context).listOpFinanceiras(
                                    IDusuario,
                                    "gastos"
                                )
                            ...
                            // Log para indicar sucesso
                            Log.d(
                                "BroadcastReceiver_Gastos",
                                "Lista de gastos atualizada com sucesso."
                            )
                        } else {
                            gastosNovos = false
                        ...
                        handler.post {
                            // Atualizações de UI ou Toast devem ocorrer aqui
                            if (gastosNovos) {
                                CompararListas_MySQL_SQLite(context).adicionarNovosGastos(
                                    DadosUsuario_BD_Debts.listas_MySQL.gastosUsuario,
                                    BancoDados(context).listaGastosMes(IDusuario)
                                )
                ...
                // Pegando o timestamp da última consulta
                val ultimaConsultaListaRendimentos: LocalDateTime =
                    DadosUsuario_BD_Debts(context).getLastUpdateTimestamp_ListaMySQL("Rendimentos")
                ...
                executorService.execute {
                    try {
                        ...
                        if (novaConsultaListaRendimentos > ultimaConsultaListaRendimentos) {
                            // Atualiza a lista de metas
                            DadosUsuario_BD_Debts.listas_MySQL.rendimentosUsuario =
                                Flask_Consultar_MySQL(context).listOpFinanceiras(
                                    IDusuario,
                                    "rendimentos"
                                )
                            ...
                            // Log para indicar sucesso
                            Log.d(
                                "BroadcastReceiver_Rendimentos",
                                "Lista de rendimentos atualizada com sucesso."
                            )
                        } else {
                            ...
                            Log.d(
                                "BroadcastReceiver_Rendimentos",
                                "Nenhuma novo rendimento encontrado."
                            )
                        }
                        ...
                        handler.post {
                            // Atualizações de UI ou Toast devem ocorrer aqui
                            if (rendimentosNovos) {
                                CompararListas_MySQL_SQLite(context).adicionarNovosRendimentos(
                                    DadosUsuario_BD_Debts.listas_MySQL.rendimentosUsuario,
                                    BancoDados(context).listaRendimentosMes(IDusuario)
                                )
            ...
            CustomToast().showCustomToast(context, "Intent recebida é nula")
            Log.d("BroadcastReceiver", "Intent recebida é nula")
        }
